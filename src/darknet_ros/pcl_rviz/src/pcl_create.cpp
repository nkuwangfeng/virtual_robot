...
const double camera_fx = 554;
const double camera_fy = 554;
...
int main(int argc, char **argv)
{
  ...
  image_transport::Subscriber sub = it.subscribe("/hsrb/head_rgbd_sensor/rgb/image_raw", 10, color_Callback);
  image_transport::Subscriber sub1 = it.subscribe("/hsrb/head_rgbd_sensor/depth_registered/image_raw", 10, depth_Callback);
 ...
 // 点云变量
  ...
  double sample_rate = 0.2; // 1HZ，1秒发1次 
  ...
  while (ros::ok()) {
    ...
    pub_pointcloud.header.frame_id = "head_rgbd_sensor_rgb_frame";
