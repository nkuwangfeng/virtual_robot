...
//const string PoseDir="/home/kamerider/catkin_sim/src/CamShift_Avatar/posedir";
//bool saveimage = false;
bool is_reach_extension=false;
std_msgs::Int16 CENTER;

ros::Publisher pub_center;
ros::Subscriber sub_is_over;

void isovercallback(const std_msgs::Bool msg)
{
	is_reach_extension = false;
}

void Extensioncallback(const std_msgs::Bool msg)
{
  cout<<"RECEIVE IS_REACH_EXTENSION"<<endl;
  is_reach_extension=msg.data;
}
...
void SIGVerseTb3OpenManipulatorGraspingAuto::personNameCallback(const std_msgs::String::ConstPtr& msg_person)
{
   person_name = msg_person->data.c_str();
}
...
bool SIGVerseTb3OpenManipulatorGraspingAuto::findGraspingTarget(geometry_msgs::Vector3 &point_cloud_pos, const std::string &target_name)
{
  for(int i=0; i<bounding_boxes_data_.boundingBoxes.size(); i++)
  {
       ...
       bounding_boxes_data_.boundingBoxes[i].probability > PROBABILITY_THRESHOLD)
    {
      ...
      if(is_succeeded) { 
		if(is_reach_extension)
        {
		  CENTER.data=center_x;
...
void SIGVerseTb3OpenManipulatorGraspingAuto::keyLoop(int argc, char** argv)
{
    ...
    std::string sub_person_topic_name;
    ...
    node_handle.param<std::string>("sub_person_topic_name",  sub_person_topic_name,   "PersonName");
    ...
    ros::Subscriber sub_point_cloud     = node_handle.subscribe(sub_point_cloud_topic_name,     100, &SIGVerseTb3OpenManipulatorGraspingAuto::pointCloudCallback, this);
    ...
    ros::Subscriber sub_person  = node_handle.subscribe(sub_person_topic_name,  10, &SIGVerseTb3OpenManipulatorGraspingAuto::personNameCallback, this);
   ...
   ros::Publisher pub_person = node_handle.advertise<geometry_msgs::Vector3>("/person/xyz", 100);
   pub_center = node_handle.advertise<std_msgs::Int16>("/center", 10);
	sub_is_over = node_handle.subscribe<std_msgs::Bool>("/Is_over",10,isovercallback);


   ros::Subscriber is_reach_extension_  = node_handle.subscribe("/Is_reach_extension",  10, Extensioncallback);
    ...
    { 
      if(!is_transforming)
      {
       ...
       if(c==GRASPING_TARGET14_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET14_NAME;
       }
       if(c==GRASPING_TARGET15_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET15_NAME;
       }
       if(c==GRASPING_TARGET16_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET16_NAME;
       }
       if(c==GRASPING_TARGET17_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET17_NAME;
       }
       if(c==GRASPING_TARGET18_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET18_NAME;
       }
       if(c==GRASPING_TARGET19_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET19_NAME;
       }
       if(c==GRASPING_TARGET20_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET20_NAME;
       }
       if(c==GRASPING_TARGET21_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET21_NAME;
       }
       if(c==GRASPING_TARGET22_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET22_NAME;
       }
       if(c==GRASPING_TARGET23_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET23_NAME;
       }
       if(c==GRASPING_TARGET24_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET24_NAME;
       }
       if(c==GRASPING_TARGET25_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET25_NAME;
       }
       if(c==GRASPING_TARGET26_NAME)
       {
       GRASPING_TARGET = GRASPING_TARGET26_NAME;
       }
      ...
      if(!is_person)
      {
        geometry_msgs::Vector3 map_target_pos_person;
        std::string e = person_name;
       if(e==PERSON)
       {    
       is_person = sensorToMap(tf_broadcaster, tf_listener, PERSON,map_target_pos_person);
       pub_person.publish(map_target_pos_person); 
       }
       }     
       else
       {
